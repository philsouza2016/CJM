    ...
    public function getListSwatchHtml($_attributes, $productId)
    {
		...
		foreach($_attributes as $_attribute):
			...
			if(in_array($_attribute['attribute_code'], $swatch_attributes)):
                ...
                $produto_simples = $_product->getTypeInstance()->getUsedProductIds();
                $cores;
               foreach( $produto_simples as $id ) {
                    $simples = Mage::getModel('catalog/product')->load($id);
                    $stock = Mage::getModel('cataloginventory/stock_item')->loadByProduct($simples);
                     if($stock->getQty()>0){
                        $cores[] = $simples->getCor();
                        echo "<span id='teste' style='display:none'>". $simples->getCor() ."</span>";
                    }
					...
					if (count($matches[0]) > 0):
                        ...
                        if(in_array($theId,$cores)){
    						if ($_product->getCjm_useimages() == 1 && $product_image ):
    							...
    							$html = $html.'" />';
    						elseif (Mage::helper('colorselectorplus')->getSwatchUrl($theId) && (in_array($theId,$cores))):
